...
import { fileExplorerContextMenu, hideFileExplorerContextMenu } from '@/stores/contextMenu';
          ...
          return (
            <React.Fragment key={idx}>
              <div className="flex flex-col px-3 py-4 bg-secondary text-xs font-semibold text-base space-y-1 tracking-wide border-b">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {state.file.type === 'folder' ? (
                      <>
                        <Icon icon="mdi:folder" width="1.2em" height="1.2em" />{' '}
                        {state.file.type}{' '}
                      </>
                    ) : (
                      <>
                        <Icon icon="mdi:file" width="1.2em" height="1.2em" />{' '}
                        <span className="truncate" title={state.file.mimeType}>
                          {state.file.mimeType}
                        </span>
                      </>
                    )}
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center min-w-[30%]">CREATED:</div>
                  <div className="flex items-center">
                    {new Date(state.file.createdAt).toLocaleString()}
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex items-center min-w-[30%]">MODIFIED:</div>
                  <div className="flex items-center">
                ...
                {state.file.type === 'folder' && state.file.children && (
                  <div className="flex items-center gap-4">
                    <div className="flex items-center min-w-[30%]">CONTENT:</div>
                    <div className="flex items-center">
