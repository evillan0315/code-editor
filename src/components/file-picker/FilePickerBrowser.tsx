...
export const FilePickerBrowser: React.FC<FilePickerBrowserProps> = ({
  ...
  const [modalCurrentDirectory, setModalCurrentDirectory] = useState<string>('.');
    ...
    if (isOpen) {
      const fetchFileTreeForModal = async () => {
        try {
          ...
          showToast(`Error fetching file tree for picker: ${err.message || String(err)}`, 'error');
  ...
  const handleSelectFileInPicker = useCallback(
    (path: string) => {
      ...
      const fileNode = modalFileTree.find((node) => node.path === path && node.type === 'file');
      if (fileNode) {
  ...
  const handleToggleFolderInPicker = useCallback(
    async (path: string) => {
      ...
      const folder = updated.find((f) => f.path === path && f.type === 'folder');
      if (folder && folder.isOpen && (!folder.children || folder.children.length === 0)) {
        try {
          const children = await fileService.list(path);
          const updatedWithChildren = toggleFolderState(path, modalFileTree, children); // Pass children to update
  ...
  const handleContextMenuInPicker = useCallback((e: React.MouseEvent, node: FileItem) => {
    ...
    >
      <div className='flex flex-col h-full'>
        ...
        <div className='modal-footer bg-secondary flex justify-end gap-2 p-4 pt-2  flex-shrink-0'>
          <Button variant='outline' onClick={handleCancelSelection} title='Cancel file selection'>
            Cancel
          </Button>
          <Button
            variant='primary'
